...
public class DebugAdvice {
	...
	//현재 클래스명은 디버그용도의 AOP를 사용했으나, AOP는 예전 자바코딩의 인터셉터(AdiviceController)와 같은 기능.
	//추가로 다중게시판용 세션관리도 AOP기능을 사용하게 됨. 코딩은 아래와 같음.
	...
	public Object sessionManager(ProceedingJoinPoint pjp) throws Throwable {
		...
		//컨트롤러 클래스에서 실행되는 모든 메서드 = (*Controller*.*(..))의 매개변수 값을 꺼네오기(아래) 향상된 for문사용
		for(Object object:pjp.getArgs()) {//예, board_update()메서드의 매개변수 Arguments
			...
			if(object instanceof PageVO) {//instanceof 객체타입을 비교하는 연산자
				...
				board_type = pageVO.getBoard_type();//세션변수로 사용할 값을 발생. jsp에서 발생한 notise,gallery
				//세션변수 발생
		...
		if(request != null) {//jsp에서 요청사항이 발생될때만 실행(아래)
			...
			if(pageVO !=null) {
				pageVO.setBoard_type(board_type);//다중게시판 쿼리추가때문에 추가
			}
			if(boardVO !=null) {
				boardVO.setBoard_type(board_type);//다중게시판 업데이트 때문에 추가
			}
		}
