...
public class ReplyController {
	...
	//댓글 수정 메서드(아래) 전송방식 POST(전통방식)가 아니고, PATCH(최근방식API)를 사용
	...
	public ResponseEntity<String> reply_write(@RequestBody ReplyVO replyVO) {
		//@RequestBody클래스는 ajax로 보내온 폼데이터를 ReplyVO 클래스에 바인딩 시켜주는 애노테이션클래스
		...
		ResponseEntity<String> result = null;
		try {//예외처리를 상위 메서드로 보내지 않는 이유는 RestAPI에서 예외 메세지를 개발자가제공하기 위해서
		...
		return result;//ResponseEntity클래스형 String값을 ajax로 호출한 페이지 반환. 
	}
