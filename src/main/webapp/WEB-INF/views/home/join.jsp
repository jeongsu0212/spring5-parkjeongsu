...
<script>
//기존회원아이디가 존재하는지 Ajax확인 후 
//신규 아이디일때만 [회원등록 버튼] 활성화 시키기, 초기엔 비활성화 시키기
jQuery(document).ready(function(){
	//$대신 jQuery를 사용하는 목적, 자바변수$ 헷갈리는것을 방지하는 효과
	//목적2. PHP기반(워드프레스나 카페24쇼핑몰같은 솔루션에서 제이쿼리를 사용할때) $사용하면 작동X. jQuery로 사용O
	//단점. $간단, jQuery 손이 힘든게 단점.
	...
	jQuery("input[name='user_id']").bind("blur",function(){
		//블러blur 이벤트는 focus선택과는 반대 이벤트=선택을 벗어났을때 이벤트
		...
		jQuery.ajax({
			...
			success:function(result){
				...
				if(result=='1'){
					alert('중복 아이디가 존재 합니다. 다시 입력해주세요.');
...
<script>
//폼submit전송시 인증대기 상태를 disabled false상태로 전송하기(아래)
/* 아래방식 대신에 다른 방식 선택
jQuery(document).ready(function(){
